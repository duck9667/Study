        ...
        map.addLayer({
            ...
            'paint': {
                'fill-color': ['case',
                    ...
                    'black',
                    generatePropertyExpression('interpolate', '설치기수', [[0, 'rgb(237,248,251)'], [100, 'rgb(191,211,230)'], [200, 'rgb(158,188,218)'], [400, 'rgb(140,150,198)'], [800, 'rgb(136,86,167)'], [1600, 'rgb(129,15,124)']], 'grey')],
                'fill-opacity': 1
            }
        }, '' );
...
plt.pie(df_sido['비율(%)'], labels=df_sido['시도'], autopct='%.1f%%')
plt.legend(loc="best")
```
    ...
    <matplotlib.legend.Legend at 0x1cda4de0df0>
...
```python
df_sido = pd.DataFrame(df.groupby(['제작사']).sum()['설치기수']).reset_index()
df_sido = df_sido.sort_values(by=['설치기수'], axis=0)
df_sido['비율(%)'] = round(df_sido['설치기수']/df_sido['설치기수'].sum(),3)*100
df_sido.head()
```
...
f, ax = plt.subplots(figsize = (16,8))
plt.xticks(rotation = 90)
plot = ax.bar(df_sido['제작사'], df_sido['설치기수'], color = sns.color_palette("BuGn_r", df_sido['제작사'].count()))
for rect in plot :
    height = rect.get_height()
    ax.text(rect.get_x() + rect.get_width()/2, height, '%d' % int(height), ha = 'center', va = 'bottom') 
```
...
```python
df_sido = pd.DataFrame(df.groupby(['종류']).sum()['설치기수']).reset_index()
df_sido = df_sido.sort_values(by=['설치기수'], axis=0)
df_sido['비율(%)'] = round(df_sido['설치기수']/df_sido['설치기수'].sum(),3)*100
df_sido.head()
```
...
f, ax = plt.subplots(figsize = (16,8))
plt.xticks()
plot = ax.bar(df_sido['종류'], df_sido['설치기수'], color = sns.color_palette("BuGn_r", df_sido['종류'].count()))
for rect in plot :
    height = rect.get_height()
    ax.text(rect.get_x() + rect.get_width()/2, height, '%d' % int(height), ha = 'center', va = 'bottom') 
```
...
```python
plt.figure(figsize = (30,8))
plt.pie(df_sido['비율(%)'], labels=df_sido['종류'], autopct='%.1f%%')
plt.legend(loc="best")
```
    ...
    <matplotlib.legend.Legend at 0x1cda2c57ac0>
...
data = pd.DataFrame(df.groupby(['시도','종류']).sum()['설치기수']).reset_index()
sns.barplot(x = '시도', y = '설치기수', hue = '종류', data=data)
```




    <AxesSubplot:xlabel='시도', ylabel='설치기수'>
...
data = pd.DataFrame(df.groupby(['시도','제작사']).sum()['설치기수']).reset_index()
sns.barplot(x = '시도', y = '설치기수', hue = '제작사', data=data)
...
```
    ...
    <matplotlib.legend.Legend at 0x1cdaf083760>
